...
def test_full_chain_upgrades_and_seeds_as_nonsuperuser_owner(migrated_db):
    ...
    assert heads == ["0032_known_people"], f"expected exactly one head at 0032, got {heads}"
...
def test_downgrade_and_reupgrade_roundtrip(migrated_db):
    ...
    assert asyncio.run(_head()) == ["0032_known_people"], "round-trip must return to head 0032"
