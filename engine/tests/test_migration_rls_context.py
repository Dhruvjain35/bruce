...
def test_full_chain_upgrades_and_seeds_as_nonsuperuser_owner(migrated_db):
    ...
    assert heads == ["0035_shadow_reconciliation"], f"expected exactly one head at 0035, got {heads}"
...
def test_downgrade_and_reupgrade_roundtrip(migrated_db):
    ...
    assert asyncio.run(_head()) == ["0035_shadow_reconciliation"], "round-trip must return to head 0035"
