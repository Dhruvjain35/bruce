...
def test_full_chain_upgrades_and_seeds_as_nonsuperuser_owner(migrated_db):
    ...
    assert heads == ["0030_claim_lineage"], f"expected exactly one head at 0030, got {heads}"
...
def test_downgrade_and_reupgrade_roundtrip(migrated_db):
    ...
    assert asyncio.run(_head()) == ["0030_claim_lineage"], "round-trip must return to head 0030"
