...
async def persist_user_turn(user_id: UUID, *, channel: str, channel_identity: str,
    ...
    async with user_session(user_id) as s:
        existing = await _turn_id(s, user_id, channel, provider_message_id, "user")
        if existing is not None:
            return existing
        row = schema.ConversationTurn(
        ...
        s.add(row)
        # FLUSHED, not merely added: the id is generated by the database default, so it does not exist
        # until the INSERT has been sent. Reading `row.id` before the flush would hand the caller None
        # and the shadow row would silently lose its canonical reference.
        await s.flush()
        return row.id
